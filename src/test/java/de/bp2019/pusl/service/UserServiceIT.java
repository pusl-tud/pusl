...
public class UserServiceIT {
    ...
    public void testDelete() throws Exception{
        ...
        user.setInstitutes(Sets.newSet(institute.getId()));
    ...
    public void testSave() throws Exception{
        ...
        wimi.setInstitutes(Sets.newSet(institute.getId()));
        ...
        hiwi.setInstitutes(Sets.newSet(institute.getId()));
        ...
        userRepository.deleteAll();
        admin.setInstitutes(Sets.newSet(institute.getId()));
    ...
    public void testFetchHiwis() throws Exception{
        ...
        User user1 = new User();
        user1.setEmailAddress(RandomStringUtils.randomAlphanumeric(10));
        user1.setInstitutes(Sets.newSet(institute.getId()));
        user1.setType(UserType.HIWI);
        userRepository.save(user1);

        User user2 = new User();
        user2.setEmailAddress(RandomStringUtils.randomAlphanumeric(10));
        user2.setInstitutes(Sets.newSet(institute.getId()));
        ...
        LOGGER.info("Testing as SUPERADMIN");
        testUtils.authenticateAs(UserType.SUPERADMIN);
        assertEquals(1, userService.fetchHiwis(new Query<>(), Sets.newSet(institute.getId())).count());

        LOGGER.info("Testing as ADMIN");
        testUtils.authenticateAs(UserType.ADMIN);
        assertEquals(0, userService.fetchHiwis(new Query<>(), Sets.newSet(institute.getId())).count());
    ...
    public void testSize() throws Exception {
        ...
        User user1 = new User();
        user1.setInstitutes(Sets.newSet(institute1.getId()));
        user1.setType(UserType.HIWI);
        userRepository.save(user1);

        User user2 = new User();
        user2.setInstitutes(Sets.newSet(institute2.getId()));
        user2.setType(UserType.HIWI);
        userRepository.save(user2);

        User user3 = new User();
        user3.setInstitutes(Sets.newSet(institute1.getId(), institute2.getId()));
        user3.setType(UserType.HIWI);
        userRepository.save(user3);

        testUtils.authenticateAs(admin);

        assertEquals(2, userService.sizeHiwis(new Query<>(), Sets.newSet(institute1.getId())));
