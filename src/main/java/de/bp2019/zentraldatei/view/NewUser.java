...
@Route("Userview/newuser") 

 public class NewUser extends Div implements HasUrlParameter<String>{
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(NewUser.class);
		...
		public NewUser(@Autowired InstituteService instituteService, @Autowired UserService userService,
				...
				usertype.setItems("Hiwi", "Wimi", "Admin" , "Superadmin");
				usertype.setValue("Hiwi");
				...
				userbinder.forField(firstname).withValidator(
						new StringLengthValidator("Bitte einen Namen eingeben", 1, null))
						.bind(User::getFirstName, User::setFirstName); 
				
				
				userbinder.forField(lastname).withValidator(
						new StringLengthValidator("Bitte einen Vornamen einbegen", 1, null))
						.bind(User::getLastName, User::setLastName);
				
				userbinder.forField(email)
						.bind(User::getEMail, User::setEMail);
				
				userbinder.forField(institutes)
						.withValidator(selectedInstitutes -> !selectedInstitutes.isEmpty(),
                                "Bitte mindestens ein Institut angeben")
						.bind(User::getInstitutes, User::setInstitutes);
				
				userbinder.forField(password).withValidator(
						new StringLengthValidator("Bitte mindestens 4 Zeichen angeben", 4, null))
